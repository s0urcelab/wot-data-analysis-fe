...
const GET_MOE_BADGE = (nation, lv) => lv < 1 ? '' : `//worldoftanks.asia/dcont/wot/current/marksOnGun/67x71/${nation}_${lv}_mark${lv > 1 ? 's' : ''}.png`;
...
function Analysis() {
  ...
  const { run: bindPlayer } = useRequest(
    (params) =>
      API('/bindUser', {
        method: 'POST',
        data: params,
      ...
      render: (name, record) => {
        ...
        return (
          ...
          >
            ...
            {pl && (
              <Space>
                <Image width={44} src={GET_MASTERY_BADGE(pl.mastery)} preview={false} />
                <Image width={44} src={GET_MOE_BADGE(nation, pl.moe)} preview={false} />
              </Space>
            )}
          </div>
        );
            ...
            <Space direction="vertical">
              ...
              <Row>
                <span style={{ fontWeight: 'bold' }}>
                  最后更新日期：{moment(data.lastUpdate).format('YYYY-MM-DD HH:mm:ss')}
                </span>
              </Row>
            </Space>
          </Card>
