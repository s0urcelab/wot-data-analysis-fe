...
import Icon, { CheckOutlined, CloseOutlined, UndoOutlined, ArrowDownOutlined, ArrowUpOutlined } from '@ant-design/icons';
import { Switch, Statistic, Input, Tooltip, Image, Space, Button, Col, Card, Radio, Row, message } from 'antd';
...
const GET_MOE_BADGE = (nation, lv) => lv < 1 ? '' : `//dav-static.worldoftanks.asia/wotpsg/wot/current/marksOnGun/67x71/${nation}_${lv}_mark${lv > 1 ? 's' : ''}.png`
...
function Analysis() {
    ...
    const {
        ...
        API('/preData', {
            method: 'GET',
            params: {
                ...(window.localStorage['bind_player_id'] && { pid: window.localStorage['bind_player_id'] }),
                ...(window.localStorage['bind_player_nickname'] && { pn: window.localStorage['bind_player_nickname'] }),
            },
        }),
    )
    ...
    const {
        run: bindPlayer,
    } = useRequest(params => API('/bindUser', {
        method: 'POST',
        data: params,
    }), {
        manual: true,
        onSuccess: (data, req) => {
            if (!data) {
                return message.error('没有找到您搜索的玩家！')
            }

            setName(req[0].nickname)
            updateSearch('player_id', data)
            window.localStorage['bind_player_id'] = data
            window.localStorage['bind_player_nickname'] = req[0].nickname
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (name, record) => {
                ...
                return (
                    <div style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} onClick={() => openModal(record)}>
    ...
    const effIndicator = (delta) => <span style={{ color: delta >= 0 ? '#3f8600' : '#cf1322' }}>{delta >= 0 ? `(↑${Math.abs(delta)})` : `(↓${Math.abs(delta)})`}</span>
    const effColor = (eff) => {
    ...
    return (
        <GridContent>
            <Suspense fallback={<PageLoading />}>
                <Row style={{ marginBottom: 24 }} gutter={24} justify="space-between" align="middle">
                    <Col>
                        <Card
                            bordered={false}
                            loading={loading}
                            title={(
                                <div className="player-header">
                                    ...
                                    {
                                        !!playerName && <Button type="primary" danger onClick={unbindPlayer}>解绑</Button>
                                    }
                                </div>
                            )}
                        >
                            {
                                !playerName && (
                                    <Input.Search
                                        ...
                                        enterButton="绑定"
                                        size="large"
                                        onSearch={val => bindPlayer({ nickname: val.trim() })}
                                    />
                                )
                            }
                            {
                                        ...
                                        <Row gutter={[16, 16]}>
                                            ...
                                            <Col span={12}>
                                                <Statistic
                                                    ...
                                                    precision={2}
                                                    suffix="%"
                                                    valueStyle={{ color: winColor(data.player.winRate) }}
                                                />
                                            </Col>
                                            <Col span={12}>
                                                <Statistic
                                                    title="个人等级"
                                                    value={data.player.wgr}
                                                />
                                            </Col>
                                            <Col span={12}>
                                                <Statistic
                                                    ...
                                                    formatter={n => <>{n} {effIndicator(data.player.effDelta)}</>}
                                        ...
                                        <Row style={{ marginTop: 24 }}>
                                            <Space>
                                                <Switch
                                                    ...
                                                    onChange={onSwChange}
                                                />
                                                <span style={{ color: '#999' }}>隐藏玩家没有的车辆</span>
                                            </Space>
                                        </Row>
                                    </>
                                )
                ...
                tableExtraRender={() => (
                    <Card>
                        <Space direction="vertical">
                            ...
                            <Row><span style={{ fontWeight: 'bold' }}>最后更新日期：{moment(data.lastUpdate).format('YYYY-MM-DD HH:mm:ss')}</span></Row>
                        </Space>
                    </Card>
                )}
